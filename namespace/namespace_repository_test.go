...
func TestIsNamespaceExistReturnTrue(t *testing.T) {
	...
	res, err := rep.isNamespaceExist(1, "test-service", "namespace")
	assert.True(t, res)
...
func TestCreateNamespaceShouldNotReturnError(t *testing.T) {
	rep := initRepository(makeMockRow(nil, nil))
	err := rep.createNewNamespace(newNamespaceStruct(1, "test-service", "namespace", 1))
	assert.NoError(t, err)
}
...
func TestCreateNamespaceShouldReturnErrorWhenDataIsEmptyFirst(t *testing.T) {
	rep := initRepository(makeMockRow(nil, nil))
	err := rep.createNewNamespace(newNamespaceStruct(0, "", "", 1))
	assert.Error(t, err)
}

func TestCreateNamespaceShouldReturnErrorWhenDataIsEmptySecond(t *testing.T) {
	rep := initRepository(makeMockRow(nil, nil))
	err := rep.createNewNamespace(newNamespaceStruct(0, "", "service", 1))
	assert.Error(t, err)
}
...
func TestRetrieveAllNamespacesShouldNotReturnErrorWhenQueryReturnNoData(t *testing.T) {
	...
	res, err := rep.retrieveAllNamespace(1, "test-service")
	assert.NoError(t, err)
...
func TestGetOrganizationIdShouldReturnId(t *testing.T) {
	rep := initRepository(makeMockRow(map[string]interface{}{"id": int64(2)}, nil))
	appcontext.InitMockContext(
		rep.execer,
	)
	res, err := rep.getOrganizationId("test-organization")
	assert.NoError(t, err)
	assert.Equal(t, 2, res)

}

func TestGetOrganizationIdShouldReturnErrorWhenNoData(t *testing.T) {
	rep := initRepository(makeMockRow(nil, nil))
	appcontext.InitMockContext(
		rep.execer,
	)
